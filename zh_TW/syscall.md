...
`argstr` 稍微特別：它先讀到的是 user 指標，接著必須把字串從 user 位址空間複製到 kernel 記憶體，這要用 `copyinstr()`，不能直接解參考。直接在 kernel 解 user pointer 是安全性漏洞。
...
- `syscall()` 結束後，`epc` 會加 4，讓 user code 從下一條指令續跑
...
這是典型的 use-after-free 資訊洩漏。正確修補是「配置時清零（zero-on-alloc）」，不是「釋放時清零（zero-on-free）」。
