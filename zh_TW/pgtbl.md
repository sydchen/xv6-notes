...
**Leaf vs. non-leaf：** 若 R/W/X 全為 0，PTE 指向下一層頁表；若 R/W/X 任一為 1，該 PTE 是 leaf，直接映射到實體頁。
...
適合回傳 **唯讀、低變動** kernel 狀態的 syscall：`getppid()`、`uptime()`、CPU 數量等。只要 kernel 能主動把值更新到共享頁，此模式就成立。
