...
`bmap(ip, bn)` 會把檔案中的邏輯區塊號（0, 1, 2, …）轉成磁碟區塊號。若區塊尚不存在，它也會按需配置。
...
要加入 doubly-indirect，且不改動磁碟 inode 大小，做法是用一個 direct slot 去交換新層級：
- `NDIRECT` 從 12 改成 11
...
鎖定協定：讀 `ip->type` 時要持有 inode lock；呼叫 `namei()` 前一定要先釋放。
...
doubly-indirect 的配置順序：root block → 第一層 block → data block，每層有修改就要呼叫 `log_write()`。任一層漏記錄，當機後都可能留下「只連一半」的 metadata。
